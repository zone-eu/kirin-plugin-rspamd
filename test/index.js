...
afterEach(async () => {
    ...
    await Promise.all(
        Array.from(servers, server => new Promise(resolve => server.close(() => resolve(undefined))))
    );
    sockets.clear();
    servers.clear();
});
...
const initialize = async (context, config) => {
    ...
    const app = {
        ...
        reject(_envelope, _description, _messageInfo, responseText) {
            ...
            const error = /** @type {Error & {responseCode: number}} */ (new Error(match ? match[2] : String(responseText)));
    ...
    await pluginModule.init(
        /** @type {import('@zone-eu/types').ZoneMtaPluginTools} */ (/** @type {unknown} */ (app)),
        () => {}
    );
    assert(dataHook);
    return {
        logs,
        run: () => /** @type {Function} */ (dataHook)(context.envelope, context.session)
    };
};

/**
 * @param {string | Record<string, unknown>} response
 * @returns {Promise<{port: number, request: Promise<Buffer>}>}
 */
const createRspamdServer = async response => {
    ...
    const server = trackServer(net.createServer(socket => {
        ...
        socket.on('data', chunk => {
            ...
            const contentLength = Number(headerText.match(/^Content-Length:\s*(\d+)$/im)?.[1] || 0);
...
describe('kirin-plugin-rspamd', () => {
    ...
    it('uses Rspamd SMTP messages for hard and soft rejects', async () => {
        ...
        await assert.rejects(hardPlugin.run(), error => isResponseError(error, 550, 'Blocked by Rspamd'));
        ...
        await assert.rejects(softPlugin.run(), error => isResponseError(error, 451, '4.7.1 Try again later'));
    });
    ...
    it('skips authenticated and private clients by default', async () => {
        ...
        assert.deepEqual(privateClient.results.get('rspamd'), { skip: 'private_ip' });
    });
    ...
    it('fails open on invalid replies unless defer.error is enabled', async () => {
        ...
        await assert.rejects(closedPlugin.run(), error => isResponseError(error, 450, 'Rspamd scan error'));
    });

    it('can defer timed-out scans', async () => {
        const server = trackServer(net.createServer({ allowHalfOpen: true }, socket => {
            socket.on('data', () => {});
        }));
        await new Promise(resolve => server.listen(0, '127.0.0.1', () => resolve(undefined)));
        const address = server.address();
        ...
        await assert.rejects(plugin.run(), error => isResponseError(error, 450, 'Rspamd scan timeout'));
