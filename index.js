...
const getConfig = input => {
    const source = /** @type {RspamdConfig} */ (/** @type {unknown} */ (input === true ? {} : input));
    ...
    return {
        ...
        rewriteSubject: { ...DEFAULT_CONFIG.rewriteSubject, ...source.rewriteSubject },
...
const buildHttpRequest = (options, body) => {
    const host = options.socketPath ? 'localhost' : `${options.host}:${options.port}`;
    const lines = [
...
const getHttpResponseBody = response => {
    ...
    return (/^transfer-encoding:\s*chunked\s*$/im.test(rawHeaders) ? decodeChunkedBody(body) : body).toString('utf8');
};
...
const requestRspamd = (options, body, timeout) =>
    new Promise((resolve, reject) => {
        const socket = options.socketPath
            ...
            : net.createConnection(/** @type {number} */ (options.port), /** @type {string} */ (options.host));
        ...
        socket.setTimeout(timeout, () => {
            const error = /** @type {Error & {code?: string}} */ (new Error('socket timeout'));
 ...
 */
const getRequestOptions = (envelope, config) => {
    /** @type {Record<string, string | string[]>} */
    const headers = {};
    const recipients = (Array.isArray(envelope.to) ? envelope.to : envelope.to ? [envelope.to] : []).map(cleanHeaderValue);
    ...
    if (envelope.originhost) headers.Hostname = cleanHeaderValue(envelope.originhost);
    ...
    if (envelope.tls && typeof envelope.tls === 'object') {
        if (envelope.tls.name) headers['TLS-Cipher'] = cleanHeaderValue(envelope.tls.name);
        if (envelope.tls.version) headers['TLS-Version'] = cleanHeaderValue(envelope.tls.version);
    }
...
const parseResponse = (rawData, app) => {
    ...
    try {
        ...
        return { error: `parse failure: ${err instanceof Error ? err.message : err}` };
    }
...
const rewriteSubject = (headers, data, config) => {
    ...
    const newSubject = String(data.subject || config.subject).replace('%s', String(oldSubject));
...
const applyMilterHeaders = (app, headers, data, config) => {
    ...
    try {
        if (data.milter.add_headers) {
            app.logger.verbose(PLUGIN_NAME, `milter.add_headers: ${JSON.stringify(data.milter.add_headers)}`);
        }
        for (const [key, input] of Object.entries(data.milter.add_headers || {})) {
            for (const headerValue of Array.isArray(input) ? input : [input]) {
                const value = headerValue && typeof headerValue === 'object' ? headerValue.value : headerValue;
...
const wantsHeadersAdded = (data, config) =>
    config.addHeaders === 'always' || (config.addHeaders === 'sometimes' && data.action === 'add header');
...
const getSmtpMessage = (data, config) => {
    if (!config.smtpMessage.enabled || !data.messages || typeof data.messages !== 'object') {
        return;
    }
...
const smtpReject = (app, envelope, code, message) =>
    app.reject(envelope, PLUGIN_NAME, '', `${code} ${String(message).replace(/^\d{3}\s+/, '')}`);
...
const scanMessage = async (app, config, envelope, session) => {
    ...
    let rawData;
    try {
        rawData = await requestRspamd(getRequestOptions(envelope, config), transaction.getMessageBuffer(), config.timeout * 1000);
    } catch (err) {
    ...
    if (config.softReject.enabled && response.data.action === 'soft reject') {
        const dsn = DSN.sec_unauthorized(smtpMessage || config.softReject.message, 451);
...
module.exports.init = async app => {
    ...
    const onSmtpData = (envelope, session) => scanMessage(plugin, config, envelope, session);
